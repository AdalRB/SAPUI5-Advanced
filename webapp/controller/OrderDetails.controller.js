sap.ui.define([
  ...
  function(Controller, History, MessageBox, Filter, FilterOperator) {
    ...
    return Controller.extend("alfa01.employeesv2.controller.OrderDetails",{
        onInit : function(){
            ...
            var customListItem = new sap.m.CustomListItem({
              content: [
                new sap.m.Bar({
                  ...
                  contentMiddle: new sap.m.ObjectStatus({text: "{i18n>availableStock} {odataNorthwind>/Products(" + contextObject.ProductID + ")/UnitsInStock}",
                                                         state: "Error"}),
        ...
        onFileBeforeUpload : function(oEvent){
          ...
          let oBindingContext = oItem.getBindingContext("odataNorthwind");
          let sOrderId = oBindingContext.getProperty("OrderID").toString();
          let sSapId = this.getOwnerComponent().SapId;
          let sEmployeeId = oBindingContext.getProperty("EmployeeID").toString();
          let sFileName = oItem.getFileName();
          let sSecurityToken = oModel.getSecurityToken();
          
          let sSlug = sOrderId+";"+sSapId+";"+sEmployeeId+";"+sFileName;

          // agregación del token
